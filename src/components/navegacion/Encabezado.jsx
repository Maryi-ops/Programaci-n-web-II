...
const cerrarSesion = async () => {
  try {
    ...
    console.error("Error cerrando sesion:", err.message);
  }
};
...
{mostrarMenu ? <t className="bi-bag-heart-fill me-2"></t> : null}
<strong>Productos</strong>
</Nav.Link >

      {/* Opción para ir al catálogo público desde admin */ }
      < Nav.Link
    onClick = {() => manejarNavegacton("/catalogo")
  }
  className = { mostrarMenu? "color-texto-marca": "text-white" }
  ...
  {/* İcono cerrar sesión en barra superior */ }
  {
    mostrarMenu ? null : (
<Nav.Link
    ...
    mostrarMenu && (
      <div className="mt-3 p-3 rounded bg-light text-dark">
        <p className="mb-2">
          ...
          {localStorage.getItem("usuario-supabase")?.toLowerCase() | I
"Usuario"}
        </p>
